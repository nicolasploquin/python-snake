...
class Cube(object):
    ...
    def draw(self, eyes=False):
        ...
        if eyes:
            centre = row_height // 2
            radius = 3
            circleMiddle = (col*row_height + centre - radius, row*row_height + 8)
            circleMiddle2 = (col*row_height + row_height - radius*2 , row*row_height+8)
...
def main():
    ...
    while running:
        ...
        positions = map(lambda c: c.pos, snake.body[1:])
        if snake.body[0].pos in positions:
            print('Score: ', len(snake.body))
            message_box('Play again...', 'Game Over!\nScore: ' + str(len(snake.body)))
            snake.reset((10,10))
        redrawWindow()
